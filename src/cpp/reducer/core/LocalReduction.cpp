...
void LocalReduction::HandleTranslationUnit(clang::ASTContext &Ctx) {
  ...
  for (auto const &FD : Functions) {
	...
	for (auto const &S : AllPrimitiveChildrenStmts) {
	  if (FilterNullElements) { //Ignore Null stmts
	    if (clang::NullStmt *NS = llvm::dyn_cast<clang::NullStmt>(S)) continue;
	  }
	  if (FilterDeclElements) { //Ignore Decl stmts
	    if (DeclStmt *DS = llvm::dyn_cast<DeclStmt>(S)) continue;
	  }
	  
	  ProgElements.emplace_back(S);
	  elem_to_fd[elem_idx] = fd_idx; //Save idx to the map
	  elem_idx += 1;
	}
      }
    }
  ...
  for (int i=0; i<prog_element_num; i++) {
    ...
    if (End.isInvalid() || Start.isInvalid()) {
      spdlog::get("Logger")->info("ProgElement {}: Location Invalid", std::to_string(i));
      continue;
    }
    else {
      llvm::StringRef SStr = SourceManager::GetSourceText(SM, Start, End);
      spdlog::get("Logger")->info("ProgElement {}: {}", std::to_string(i), SStr.str());
    }
  }
